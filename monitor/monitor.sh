...
#判断第一个参数不为空 -z 在if里的意思是空 最后结果是 if[1]或者if[0]
if [[ ! -z $iopt ]]  
then
{
wd=$(pwd)

#该文件名是否存在且为一个连接文件  readlink 主要用来找出符号链接所指向的位置.
basename "$(test -L "$0" && readlink "$0" || echo "$0")" > /tmp/scriptname
scriptname=$(echo -e -n $wd/ && cat /tmp/scriptname)
echo $scriptname
su -c "cp $scriptname /usr/bin/linzijun" root && echo "Congratulations! Script Installed, now run monitor Command" || echo "Installation failed"
}
fi
...
then
{
echo "uninstall linzijun"
sudo rm /usr/bin/linzijun   
}
fi
